$(document).ready(function() {
	...
	$(document).on('click', '#add_member', function(event) {
		...
		if(! name ){
			alert('Please give a Member Name.');
			return false;
		}
		if(! father_name ){
			alert('Please give Husband or Father Name.');
			return false;
		}
		if(! address ){
			alert('Please give Village.');
			return false;
		}
		if(! share ){
			alert('Please give Share Amount.');
			return false;
		}
	...
	$(document).on('change', '#member_code', function(event) {
		...
		$.ajax({
			type:"GET",
			url: "loaner/member_details/"+member,
			dataType : "json",
			...
			success: function(result) { 
				...
				$("#name").val(mem.name);
				$("#father_name").val(mem.father_name);
				$("#village").val(mem.village);
				$("#post_office").val(mem.post_office);
				$("#police_station").val(mem.police_station);
				$("#district").val(mem.district);
	...
	$(document).on('keyup', '#loan_amount', function(event) {

		var loan_amount = $('#loan_amount').val();
		var service_charge = (loan_amount/100)*20;
	...
	$(document).on('change', '#loan_code', function(event) {
